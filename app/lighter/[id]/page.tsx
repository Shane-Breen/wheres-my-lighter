...
export default function LighterJourneyPage({
  ...
  // --- Demo-safe values (swap for real data when ready) ---
  const lighterId = params?.id ?? 'pilot-002'
  ...
  // --- Scroll fade for identity row ---
  const { scrollY } = useScroll()
  const identityOpacity = useTransform(scrollY, [0, 120], [1, 0.4])

  // --- Distance + microcopy reveal ---
  const [distanceKm, setDistanceKm] = React.useState<number>(1243)
  const [hasTapped, setHasTapped] = React.useState(false)

  const handleLogTap = () => {
    // Replace with your real "log tap" flow / API call
    setHasTapped(true)
    setDistanceKm((prev) => prev + Math.floor(Math.random() * 5 + 1))
  }

  return (
    <main className="min-h-screen w-full bg-[#070817] text-white">
      ...
      <div className="relative mx-auto w-full max-w-[420px] px-5 pb-24 pt-10">
        {/* Header */}
        ...
        >
          <div className="text-xs text-white/70">{lastSeen.title}</div>
